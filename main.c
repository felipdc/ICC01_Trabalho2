...
const char *stats_names[] = {"name", "test_grade", "work_grade", 
							"work_pos"};
...
int get_stat_code (const char *goal_name) {
	if (strcmp (goal_name, stats_names[0]) == 0) {
		return NAME_ATRIB;
	}
	if (strcmp (goal_name, stats_names[1]) == 0) {
		return TEST_ATRIB;
	}
	if (strcmp (goal_name, stats_names[2]) == 0) {
		return WORK_G_ATRIB;
	}
	if (strcmp (goal_name, stats_names[3]) == 0) {
		return WORK_P_ATRIB;
	}
}
